...
const ReactCompareImage = (props) => {
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(([entry, ..._]) => {
      ...
      const currentContainerHeight = entry.target.getBoundingClientRect().height;
  ...
  useEffect(() => {
    ...
    if (allImagesLoaded) {
      ...
      // const rightImageWidthHeightRatio = rightImageRef.current.naturalHeight
      //           / rightImageRef.current.naturalWidth;
      // const idealWidthHeightRatio = aspectRatio === 'taller'
  ...
  const styles = {
    ...
    slider: {
      alignItems: 'center',
      cursor: (!hover && horizontal && 'ew-resize')
